def convertir(T):
    ...
    130
    """
    assert isinstance(T, list) and T != [], 'Merci de spécifier un tableau non vide avec comme valeurs du binaire (0 ou 1)'
    final = 0
    puissance = len(T) - 1
    for nombre in T:
        assert isinstance(nombre, int) and (nombre == 0 or nombre == 1)
        if(nombre == 1):
            final += 2 ** puissance
        puissance -= 1
    return final
