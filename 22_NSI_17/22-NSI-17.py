def nombre_de_mots(phrase: str) -> int:
    ...
    taille = len(splited)
    if('?' in splited or '!' in splited): return taille - 1
    elif('.' in splited[-1]): return len(taille)
    else:
        raise ValueError('La phrase ne se termine pas, ni avec un \'.\' collées avec les derniers mots ou bien meme avec un \'?\, ou \'!\' séparés du dernier mots !')
